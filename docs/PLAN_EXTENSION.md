...
| BP | Business Plan détaillé + PPT 3-4 slides + vidéo 1min | ⚠️ Business Plan détaillé (17 sections, SAM réel 650 chiffré) + **PPT passé à 5 slides** (Problème/Solution séparées + vraie Conclusion, sur demande explicite utilisateur — dépasse le "3-4 max" de `docs/CONCOURS.md`, à confirmer) avec habillage visuel étendu (cartes verre dépoli, glow néon, accents isométriques, réseau sur la mini-carte des 6 villes CESTOM), vérifié par `pitch/verify_deck.py` (a trouvé et fait corriger 2 vrais défauts : chevauchement de labels, crash d'encodage du script lui-même). **Confirmé : aucun outil de génération d'images/vision par ordinateur disponible ici (vérifié 2×) — les visuels "3D isométrique" demandés sont un habillage vectoriel pptxgenjs, pas des rendus IA photoréalistes.** Restent : "ask" au jury, relecture visuelle humaine (jamais vu à l'écran), vidéo 1min. Échéance dure 2026-09-03 23:59 |
