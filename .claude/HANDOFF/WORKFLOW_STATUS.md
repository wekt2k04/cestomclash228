...
| Accueil séparé avant la carte (`WelcomeIntro`) | 2026-09-05 — **confirmé visuellement via Claude in Chrome réel** (rendu inspecté à l'écran, pas juste SSR/curl). A revele une faille critique (texte de bouton invisible sans hover, `a{}` hors @layer) corrigée le jour même | ✅ (rendu confirmé, voir LOG.md) |
| Contraste bouton "Rejoindre" (Link stylisé) | 2026-09-05 — **faille critique trouvée et corrigée** : texte invisible sans hover (`a { color }` hors @layer écrasait `text-terracotta-ink`). Revérifié à l'écran après fix | ✅ |
| Carte Maroc — variation de couleur par ville | 2026-09-05 — était entièrement monochrome orange (retour "mal aux yeux"), 3 couleurs de marque introduites par palier de taille | ⚠️ build/tsc vert + un aperçu visuel avant clic confirmé, mais le clic "Explorer la carte" n'a pas répondu de façon fiable en test automatisé (voir LOG.md) — à confirmer par l'utilisateur |
| Musique — lien de crédit + volume | 2026-09-05 — lien reformulé ("Musique (crédit)"), volume par défaut relevé 0.18→0.32, piste remplacée (ambient.mp3) | ⛔ toujours non confirmé sur téléphone réel (limite structurelle inchangée) |
| 9 bugs mobile confirmés (cibles tactiles carte, zoom iOS formulaires, safe-area, scroll sheets, error boundaries, reprise audio) | 2026-09-05 — séquence A→H complète, validée par un agent Plan dédié avant exécution (3 découvertes ajoutées au périmètre : viewport-fit=cover prérequis, padding-top Header, global-error.tsx en plus d'error.tsx) | ⚠️ tsc/lint verts + SSR réel confirme `viewport-fit=cover` fusionné et les 2 fichiers error boundary enregistrés dans l'arbre de routage (curl sur serveur dev réel) — rendu visuel effectif sur appareil réel (encoche, reprise audio après verrouillage, collision Rabat/Casablanca à l'œil) reste ⛔, pas d'extension navigateur connectée lors de cette vérification |
